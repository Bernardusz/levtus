 ...
 * <p>
 * Provides a fluent interface to access the HTTP method, URI path, headers, query parameters,
...
public class Request {
     ...
     *
     * @implNote This constructor is primarily used internally by the Levtus engine during the HTTP parsing phase.
     *
    ...
    /**
     * Retrieves all values associated with a specific HTTP header.
     * Header name resolution is case-insensitive.
     * <p>
     * {@code List<String> accepts = req.getHeaders("Accept");}
     *
    ...
    /**
     * Determines the MIME type of the request payload based on the 'Content-Type' header.
     * Defaults to "text/plain" if the header is missing.
     *
    ...
    /**
     * Parses the 'Content-Length' header to determine the expected size of the request body.
     * Defaults to 0 if the header is missing or malformed.
     *
    ...
    /**
     * Lazily reads the incoming payload from the raw stream into a byte array and caches it.
     * Enforces the configured maximum body size limits to prevent memory exhaustion (OOM) attacks.
     *
     * @return the raw byte array of the request body
     * @throws PayloadTooLargeException if the 'Content-Length' or actual stream data exceeds {@code maxBodySize}
